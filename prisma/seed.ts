...
async function main() {
  ...
  await prisma.sourceCoverage.createMany({
    data: SOURCE_CONNECTORS.map((source) => ({
      sourceName: source.name,
      sourceType: source.sourceType,
      baseUrl: source.baseUrl,
      rssUrlsJson: source.rssUrls as unknown as Prisma.InputJsonValue,
      enabled: source.enabled,
      reliabilityScore: source.reliabilityScore,
      allowedCategories: source.allowedCategories.join(", "),
      blockedKeywords: source.blockedKeywords.join(", "),
      preferredKeywords: source.preferredKeywords.join(", "),
      lastCheckedAt: date("2026-04-27"),
      lastSuccessfulFetchAt: date("2026-04-27"),
      articlesFetchedLastRun: source.name === "Deadline" ? 16 : source.name === "Variety" ? 12 : 0,
      articlesSavedLastRun: source.name === "Deadline" ? 5 : source.name === "Variety" ? 3 : 0,
      articlesExcludedLastRun: source.name === "Deadline" ? 8 : source.name === "Variety" ? 6 : 0,
      highRelevanceCountLastRun: source.name === "Deadline" ? 4 : source.name === "Variety" ? 2 : 0,
      mediumRelevanceCountLastRun: source.name === "Deadline" ? 1 : source.name === "Variety" ? 1 : 0,
      lowRelevanceCountLastRun: source.name === "Deadline" ? 11 : source.name === "Variety" ? 9 : 0,
      commonExclusionReasons: source.name === "Deadline" ? "Blocked by keyword: leadership summit (2)" : source.name === "Variety" ? "Low-signal or unrelated entertainment coverage (3)" : null,
      failuresLast7Days: 0,
      sourceReliability: inferSourceReliability(source.name, source.baseUrl),
      notes: source.notes
    }))
  });
