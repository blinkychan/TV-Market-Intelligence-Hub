import { Activity, Database, FileText, Plus, Radio, Search, Upload } from "lucide-react";
import { addManualArticle, runMockRssIngestion, runRssIngestion, saveBackfillRequest, saveRssFeed } from "./actions";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Card className="shadow-panel">
          <CardHeader>
            <CardTitle>Backfill Search Placeholder</CardTitle>
            <p className="text-sm text-muted-foreground">Save structured search requests for 2023-onward coverage without running retrieval yet.</p>
          </CardHeader>
          <CardContent>
            <form action={saveBackfillRequest} className="grid gap-3 md:grid-cols-2">
              <Input name="source" placeholder="Source" disabled={dataSource === "mock"} />
              <Input name="statusCategory" placeholder="Status / category" disabled={dataSource === "mock"} />
              <Input name="month" placeholder="Month" disabled={dataSource === "mock"} />
              <Input name="year" placeholder="Year" disabled={dataSource === "mock"} />
              <textarea
                name="keywords"
                rows={4}
                placeholder="Keywords"
                className="md:col-span-2 w-full rounded-md border bg-background px-3 py-2 text-sm"
                disabled={dataSource === "mock"}
              />
              <div className="md:col-span-2 flex items-center justify-between gap-3">
                <p className="text-sm text-muted-foreground">This stores the request as an ingestion-history draft for later chunked processing.</p>
                <Button type="submit" disabled={dataSource === "mock"}>
                  <Search className="h-4 w-4" /> Save Backfill Request
                </Button>
              </div>
            </form>
          </CardContent>
      ...
      <Card className="shadow-panel">
        ...
        <CardContent className="grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-lg border bg-slate-50 p-4 text-sm text-muted-foreground">
            Backfill requests are still placeholders, keeping this step scoped to RSS ingestion plus review-queue routing.
          </div>
        </CardContent>
      </Card>
    </div>
